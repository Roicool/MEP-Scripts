/**
 * Entrance 1.0.2 — Scroll reveal animasyonu
 ...
 *
 * 1.0.1: gsap.registerPlugin(ScrollTrigger) idempotent çağrısı eklendi.
 *        Init sonrası ScrollTrigger.refresh() — başka scriptlerle senkron.
 * 1.0.2: left/right yönlerinde x:±52px elementi viewport dışına taşıyarak
 *        layout shift yapıyordu. html elementine overflowX:clip eklendi.
 *        overflow:clip yeni scroll context oluşturmaz — Lenis etkilenmez.
 */
(function () {
  'use strict';
  ...
  function init() {
    ...
    /* left/right yönlerinde yatay overflow kaymasını önle */
    var hasHorizontal = false;
    els.forEach(function (el) {
      var dir = el.getAttribute('data-entrance') || 'bottom';
      if (dir === 'left' || dir === 'right') { hasHorizontal = true; }
    });
    if (hasHorizontal) {
    ...
    /* Diğer scriptlerin DOM mutasyonları sonrası start pozisyonları yeniden hesaplansın */
