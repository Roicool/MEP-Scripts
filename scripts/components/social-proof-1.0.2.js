...
(function () {
  ...
  var BP_MOB     = 992;   /* mobil breakpoint (px)      */
  ...
  function init() {
    ...
    function resetContent(c) {
      ...
      gsap.set(c, { opacity: 0, y: 0, pointerEvents: 'none' });
    ...
    function goTo(index) {
      ...
      gsap.set(prevCnt, { pointerEvents: 'none' });
      ...
      gsap.fromTo(nextCnt,
        { opacity: 0, y: 28 },
        {
          ...
          overwrite: 'auto',
          onStart:    function () {
            nextCnt.classList.add('is-active');
            gsap.set(nextCnt, { pointerEvents: 'auto' });
          },
    ...
    /* ── Mobil: swipe ile slide geçiş ───────────────────── */
    var touchStartX = 0;
    section.addEventListener('touchstart', function (e) {
      touchStartX = e.changedTouches[0].clientX;
    }, { passive: true });
    section.addEventListener('touchend', function (e) {
      if (window.innerWidth >= BP_MOB) return;
      var dx = e.changedTouches[0].clientX - touchStartX;
      if (Math.abs(dx) < 40) return;
      goTo(dx < 0 ? (current + 1) % ns : (current - 1 + ns) % ns);
    }, { passive: true });
