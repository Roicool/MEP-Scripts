/**
 * FSC 1.0.1 — Fullscreen Section Cases
...
(function () {
  ...
  function init() {
    ...
    function goTo(index) {
      ...
      contents.forEach(function (c, i) {
        ...
        if (i !== prev && i !== index) {
          ...
          gsap.set(c, { opacity: 0, y: 0, rotateX: 0, scale: 1, pointerEvents: 'none' });
      ...
      videos.forEach(function (v, i) {
        ...
        if (i !== prev && i !== index) {
          ...
          gsap.set(v, { opacity: 0, y: 0, scale: 1, pointerEvents: 'none' });
      ...
      gsap.to(prevVid, {
        ...
        onComplete: function () {
          ...
          gsap.set(prevVid, { y: 0, scale: 1, rotateX: 0, pointerEvents: 'none' });
        },
      });
      gsap.fromTo(nextVid,
        { opacity: 0, y: -20, scale: 0.96, rotateX: -6 },
        {
          ...
          ease:     'power3.out',
          overwrite: 'auto',
          onStart: function () { nextVid.classList.add('is-active'); gsap.set(nextVid, { pointerEvents: 'auto' }); },
      ...
      gsap.to(prevCnt, {
        ...
        onComplete: function () {
          ...
          gsap.set(prevCnt, { y: 0, scale: 1, rotateX: 0, pointerEvents: 'none' });
        },
      });
      gsap.fromTo(nextCnt,
        { opacity: 0, y: 20, scale: 0.97, rotateX: -5 },
        {
          ...
          ease:     'power3.out',
          overwrite: 'auto',
          onStart: function () { nextCnt.classList.add('is-active'); gsap.set(nextCnt, { pointerEvents: 'auto' }); },
    ...
    /* tab tıklama — aktif değilse geçiş yap (link navigasyonunu engelle);
       aktifse link çalışsın */
    ...
    /* ── Init state ──────────────────────────────────────── */
    gsap.set(bgs,      { opacity: 0, zIndex: 0, pointerEvents: 'none' });
    gsap.set(videos,   { opacity: 0, pointerEvents: 'none' });
    gsap.set(contents, { opacity: 0, pointerEvents: 'none' });
    gsap.set(fills,    { scaleX: 0, transformOrigin: 'left center' });

    if (bgs[0])      { gsap.set(bgs[0],      { opacity: 1, zIndex: 1, pointerEvents: 'auto' }); bgs[0].classList.add('is-active'); }
    if (videos[0])   { gsap.set(videos[0],   { opacity: 1, pointerEvents: 'auto' }); videos[0].classList.add('is-active'); }
    if (contents[0]) { gsap.set(contents[0], { opacity: 1, pointerEvents: 'auto' }); contents[0].classList.add('is-active'); }
